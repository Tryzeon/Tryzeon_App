...
-- picks exactly one; multi-tagging only polluted shopper-side browsing and
-- forced union logic in the size-measurement UI.
--
-- Verified on prod 2026-07-24: 337 products all have >= 1 category and every
-- category_ids[1] is a valid product_categories id; 6 rows had 2 categories
-- (converged to the first element — store owners can re-pick in the edit page).
--
-- The p_category_ids FILTER param stays text[]: shopper-side filtering remains
-- multi-select. Both callers (app picker, chat edge function) source ids from
-- product_categories, so the ::uuid[] cast below is safe.

ALTER TABLE "public"."products"
  ADD COLUMN "category_id" "uuid" REFERENCES "public"."product_categories"("id");

UPDATE "public"."products"
  SET "category_id" = "category_ids"[1]::"uuid";

ALTER TABLE "public"."products"
  ALTER COLUMN "category_id" SET NOT NULL;

-- Drops idx_products_category_ids (GIN) together with the column.
ALTER TABLE "public"."products" DROP COLUMN "category_ids";

CREATE INDEX "idx_products_category_id"
  ON "public"."products" USING "btree" ("category_id");

-- Rebuild list_shop_products: payload key category_ids -> category_id, overlap
-- filter -> single-value membership. Same body as
-- 20260723010000_rename_product_variants_to_product_sizes.sql otherwise.
