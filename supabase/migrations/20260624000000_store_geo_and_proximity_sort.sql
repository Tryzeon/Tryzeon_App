-- Add store coordinates and proximity ("nearest") sort to list_shop_products.
-- Coordinates are populated client-side via native geocoding when a store owner
-- edits their address. Proximity sort orders by great-circle (Haversine) distance
-- in metres, ascending, with un-geocoded stores last.

ALTER TABLE "public"."store_profiles"
  ADD COLUMN IF NOT EXISTS "latitude"  double precision,
  ADD COLUMN IF NOT EXISTS "longitude" double precision;

-- Adding two parameters changes the function signature, so drop the prior
-- 17-arg signature and recreate with 19 args (p_user_lat, p_user_lng added).
