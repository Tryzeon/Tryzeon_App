-- Make material filtering multi-select: p_material (text, single ILIKE) becomes
-- p_materials (text[], OR-of-ILIKE) so the consumer can filter by several
-- materials at once (棉 OR 麻 ...), consistent with the other multi-select
-- attribute filters. Changing a parameter's type/name changes the function
-- signature, so we DROP the prior 17-arg signature and recreate it.
-- Everything else (other filters, ordering, pagination, indexes) is unchanged.
-- The old get_shop_products function remains retained (dropped later, post-review).
